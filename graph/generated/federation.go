	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Foo", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Foo", ErrTypeNotFound))
			break
		}
		return "findFooByID", nil
	}
	return "", fmt.Errorf("%w for Foo due to %v", ErrTypeNotFound,
